 ...
 * в UpdateProfileRequest этих полей просто нет, поэтому даже подделанный
...
function bindForm() {
    $('profile-form').addEventListener('submit', async (event) => {
        ...
        try {
            ...
            // действием (клик по фото → загрузка файла или камера), а не
            // через это поле формы — совмещать оба пути только путало бы.
...
async function openCameraCapture(dialog) {
    ...
    // Камеру нужно выключить в любом случае, когда окно закрывается —
    // иначе индикатор камеры в браузере продолжит гореть после закрытия.
...
startStudentPage(load, { init: () => { bindForm(); bindAvatarClick(); } });
